...
use probe::deep::StreamProbeResult;
  ...
  for i in audio_indexes {
    ...
    filters.push(
      Filter {
        ...
        inputs: Some(vec![
          FilterInput { kind: InputKind::Stream, stream_label: input_identifier }
        ]),
        outputs: None
      });
    filters.push(
      Filter {
        ...
        inputs: None,
        outputs: Some(vec![
          FilterOutput { stream_label: output_identifier.clone() }
        ])
      });
...
pub fn detect_silence(filename: &String, streams: &mut Vec<StreamProbeResult>, audio_indexes: Vec<u32>) -> () {
          ...
          Entry(entry_map) => {
            if let Some(stream_id) = entry_map.get("stream_id") {
              ...
              if let Some(value) = entry_map.get("lavfi.silence_start") {
                streams[(index) as usize].silence_start.push(value.to_string());
              }
              if let Some(value) = entry_map.get("lavfi.silence_end") {
                streams[(index) as usize].silence_end.push(value.to_string());
              }
            }
          },
          _ => {},
